load(file = "../Analysis/resList_UACC257.RData")
...
for(ii in 1:length(resList)){
  ...
  resultSIF_tp5$Y<-"90min"
  int<-intersect(resultSIF_tp5$X, resultSIF_tp4$X)
  resultSIF_tp5$Y[which(match(resultSIF_tp5$X, int) >0)] <- "60min"
  
  int<-intersect(resultSIF_tp4$X, resultSIF_tp3$X)
  resultSIF_tp5$Y[which(match(resultSIF_tp5$X, int) >0)] <- "30min"
  
  int<-intersect(resultSIF_tp3$X, resultSIF_tp2$X)
  resultSIF_tp5$Y[which(match(resultSIF_tp5$X, int) >0)] <- "10min"
  
  int<-intersect(resultSIF_tp2$X, resultSIF_tp1$X)
  resultSIF_tp5$Y[which(match(resultSIF_tp5$X, int) >0)] <- "2min"
  
  result<-cbind(resultSIF_tp5[,1],resultSIF_tp5[,5],resultSIF_tp5[,3])
  colnames(result)<-c("Source","Entry","Target")
...
for(i in 1:nrow(uniqueAttribSIF)){
  ...
  if(length(idx)==1){
    ...
    tp1<-attribSIF[idx[which(attribSIF[idx,2]=="2min")],]
     ...
     tp3<-attribSIF[idx[which(attribSIF[idx,2]=="30min")],]
     tp4<-attribSIF[idx[which(attribSIF[idx,2]=="60min")],]
     tp5<-attribSIF[idx[which(attribSIF[idx,2]=="90min")],]
     ...
     if(as.numeric(tp1[4]) > 20){uniqueAttribSIF[i, 2]<-tp1[2]} 
      ...
      if((as.numeric(tp2[4]) > 20)){uniqueAttribSIF[i, 2]<-tp2[2]}
        ...
        else {uniqueAttribSIF[i, 2]<-tp5[2]}
       }
        }
      }
    # ts<-matrix(attribSIF[idx,],ncol = 4)
    # ts<-cbind(ts, gsub("min","",ts[,2]))
    # ts<-ts[order(as.numeric(ts[,5],decreasing= TRUE)),]
    # ts[which(abs((as.numeric(ts[,4]))-mean(as.numeric(ts[,4])))==min(abs((as.numeric(ts[,4]))-mean(as.numeric(ts[,4]))))),2]
    # ts<-ts[1,]
    # uniqueAttribSIF[i, 2] <- ts[2] 
       }
  
}

write.table(x = uniqueAttribSIF, file = "inst/tpBootstrapAnalysis_UACC257.txt", quote = FALSE, sep = "\t", row.names = FALSE)