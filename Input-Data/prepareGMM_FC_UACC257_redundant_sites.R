...
wtQ <- Endothelin_ShortTerm_UACC257[, c("WT_2min q", "WT_10min q", "WT_30min q", "WT_60min q", "WT_90min q")] # retrieving the q-values

rNames <- paste0(Endothelin_ShortTerm_UACC257$"Uniprot accession", "_", Endothelin_ShortTerm_UACC257$Site, "_", Endothelin_ShortTerm_UACC257$"n PO4")
...
data.IDmap[, 1] <- rNames
data.IDmap[, 2] <- Endothelin_ShortTerm_UACC257$"Uniprot accession"
data.IDmap[, 3] <- paste0(Endothelin_ShortTerm_UACC257$"Uniprot accession", "_", Endothelin_ShortTerm_UACC257$Site)

###
# Preparing values for GMM object

# log2 transforming the fc's
numData.Avg <- matrix(, nrow = nrow(wtFC), ncol = 5)
...
allNumData <- c(as.numeric(numData.Avg[, 1]), as.numeric(numData.Avg[, 2]), as.numeric(numData.Avg[, 3]), as.numeric(numData.Avg[, 4]), as.numeric(numData.Avg[, 5]))
...
threshQ <- 0.1 # defining threshold q-value
...
names(tp1.s) <- rownames(numData.Avg)
tp1.s[which(min > -0.58)] <- "FP"            # giving FP status to measurements with lower fc than 1.5 and OK status to measurement with higher fc than 1.5
...
for(i in 1:length(GMM)){
  ...
  rownames(GMM[[i]])<-c("tp_2min", "tp_10min", "tp_30min", "tp_60min", "tp_90min")
}
...
for(i in 1:length(GMM.wFC)){
  ...
  rownames(GMM.wFC[[i]])<-c("tp_2min", "tp_10min", "tp_30min", "tp_60min", "tp_90min")
}
...
save(list=c("GMM.ID", "GMM","GMM.wFC"), file="dataGMM_UACC257.RData")
