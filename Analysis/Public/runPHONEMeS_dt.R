  ...
  for(ii in 1:nIter){
    ...
    for(jj in 1:length(experiments)){
      ...
      if(jj==1){
        ...
        cascade=list(c1=c("PI3K_HUMAN"),c2=c("PDPK1_HUMAN"), c3=c("AKT_HUMAN"))
        ...
        # pknList <- buildNetwork(experiments=experiments, data.P=data.P, bg=bg, nK = nK, temp = tempSIF)
        ...
        if (solver=="cplex"){
          resultsSIF1 <- solve_with_cplex()
          tempSIF = unique(rbind(tempSIF, resultsSIF1))
        } else if (solver=="cbc"){
          resultsSIF1 <- solve_with_cbc()
        } else {
          stop("Select a valid solver option ('cplex', 'cbc')")
...
solve_with_cplex <- function(){
  ...
  if (Sys.info()[1]=="Windows"){
    ...
    system(paste0(getwd(), "/cplex.exe -f cplexCommand.txt"))
